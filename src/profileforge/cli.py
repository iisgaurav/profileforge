...
from profileforge.core.registry import WIDGET_REGISTRY, DATASOURCE_REGISTRY
...
def cmd_build(args):
    ...
    try:
        ...
        out_dir = Path(args.output) if args.output else config_path.parent / config.outputs.svg.dir
    ...
    try:
        ...
        for w in config.widgets:
            if w.name in WIDGET_REGISTRY:
                ...
                print(f"\nReason:\nUnknown widget")
                print(f"\nDid you mean:\n" + "\n".join([f"• {k}" for k in WIDGET_REGISTRY.keys()]))
                sys.exit(1)
        ...
        if os.access(out_dir, os.W_OK):
            print_success(f"Output directory writable")
        else:
...
def main():
    parser = argparse.ArgumentParser(description="ProfileForge: Open-Source GitHub Profile Engine")
    subparsers = parser.add_subparsers(dest="command", required=True)
    
    build_parser = subparsers.add_parser("build", help="Build the profile widgets")
    build_parser.add_argument("--config", default="profileforge.yaml", help="Path to config file")
    build_parser.add_argument("--output", default=None, help="Override output directory")
    
    subparsers.add_parser("doctor", help="Check dependencies and environment")
    
    new_parser = subparsers.add_parser("new", help="Scaffold a new ProfileForge project")
    new_parser.add_argument("name", help="Name of the new project directory")
    
    validate_parser = subparsers.add_parser("validate", help="Validate a configuration file")
    validate_parser.add_argument("--config", default="profileforge.yaml", help="Path to config file")
    validate_parser.add_argument("--output", default=None, help="Output directory to check write access")
