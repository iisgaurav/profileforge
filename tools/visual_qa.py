...
def run_gallery_export(out_dir):
    ...
    result = subprocess.run(["profileforge", "gallery", "export", "--out-dir", out_dir], capture_output=True, text=True)
...
class SvgAccessibilityValidator:
    ...
    def validate(cls, root: ET.Element) -> list[str]:
        ...
        if not has_required_attr:
            issues.append(f"Missing A11y tags or attributes: {', '.join(cls.REQUIRED_ATTRIBUTES)}")
...
def main():
    ...
    parser.add_argument("--baseline-dir", default="artifacts/baseline", help="Baseline gallery directory")
    parser.add_argument("--current-dir", default="artifacts/current", help="Current gallery directory")
    parser.add_argument("--diff-dir", default="artifacts/diff", help="Diff output directory")
    parser.add_argument("--require-qa", action="store_true", help="Fail if differences exist and DESIGN_QA.md is missing")
