...
def run_benchmark():
    ...
    result = subprocess.run(["profileforge", "gallery", "export", "--out-dir", "artifacts/benchmark"], capture_output=True, text=True)
    ...
    svg_count = len([f for f in os.listdir(assets_dir) if f.endswith('.svg')]) if os.path.exists(assets_dir) else 0
    
    metrics = {
        "build_time_seconds": round(duration, 3),
        "svg_count": svg_count,
        "avg_render_time_ms": round((duration / svg_count * 1000), 2) if svg_count > 0 else 0
    }
